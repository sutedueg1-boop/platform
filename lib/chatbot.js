 ...
 */
function handleMessage({ text, user, data, context }) {
  ...
  const layers = [
    ...
    // أسئلة "إزاي أستخدم فيتشر كذا؟" (إشعارات، استهداف المحاضرات، طلب
    // محاضرة، تسجيل واختبار...) — إضافة 15 سبتمبر 2026. لازم تيجي قبل
    // طبقتي البيانات الشخصية والتحليلات، لأن سؤال إجرائي زي "أعمل اختبار
    // للمحاضرة إزاي؟" كان بيتلخبط مع سؤال أرقام زي "كام محاضرة؟" ويرجع
    // إحصائية عامة بدل شرح الفيتشر (المشكلة اللي بشمهندس أحمد لاحظها:
    // "بيرد ردود عامة أوي").
  ...
  for (const layer of layers) {
    ...
    if (r) return { source: null, suggestions: r.suggestions || [], topic: r.topic || null, ...r };
  }
