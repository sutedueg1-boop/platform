...
/**
 * بيقرا رقم **بس لو الإجابة هي الرقم نفسه** ("3" / "رقم 3" / "التالت").
 * (22 سبتمبر 2026) قبل كده كان بياخد أول رقم في أي جملة: "عايز اعمل بلاغ
 * في مكان ال 2 ... خطورتها عالي جدا" اتقرت "احتمالية ضئيلة" (2)! الرقم
 * كان جزء من اسم المكان مش إجابة.
 */
function pickIndex(text, max) {
  const t = N(text).trim().replace(/[.,،!؟?]/g, '').trim();
  const bare = t.match(/^(?:رقم\s*|اختار\s*|الاختيار\s*)?(\d{1,2})$/);
  if (bare) {
    const n = parseInt(bare[1], 10);
    if (n >= 1 && n <= max) return n - 1;
  }
  const ORD = ['الاول', 'التاني', 'التالت', 'الرابع', 'الخامس', 'السادس', 'السابع', 'التامن', 'التاسع'];
  // الترتيب برضه لازم يكون هو الإجابة كلها تقريبًا (مش كلمة جوه جملة)
...
function answerFollowup(text, ctx) {
  ...
  // (22 سبتمبر 2026) رسالة جديدة مش إجابة: طلب جديد ("عايز اعمل بلاغ...")
  // أو سؤال ("...اعمل اي") → نسيب السؤال المعلّق خالص ونرد على الرسالة
  // دي عادي. قبل كده كان المستخدم بيتحبس: أي حاجة يكتبها ترجعله "مش فاهم
  // الإجابة دي" ونفس السؤال تاني. (null هنا = الطبقات العادية ترد،
  // والرد الجديد مش هيحمل السؤال القديم فالواجهة بتنساه لوحدها.)
  const nRaw = N(raw);
  const looksNew = /(عايز|عاوز|محتاج|ممكن)\s*(اعمل|اطلع|ابلغ|افتح|اقدم)?\s*(بلاغ|تصريح|تصاريح)/.test(nRaw)
    || /(اعمل|نعمل|اتصرف)\s+(اي|ايه|ازاي)|ازاي|ليه|امتي|مين|فين|كام|ايه\s|(^|\s)لو\s(?!سمحت)|[؟?]/.test(nRaw)
    || /^(وديني|ودني|افتح|خدني|روح)/.test(nRaw);

  const value = looksNew ? null : def.parse(raw, ctx);
  if (value === null || value === undefined) {
    if (looksNew) return null;
    // إجابة قصيرة مش مفهومة (زي "مم" أو "اه") → نعيد السؤال مرة واحدة بس.
    // أي حاجة أطول من كده غالبًا كلام جديد → نسيبها للطبقات العادية.
