...
function bidi(v) {
  ...
  while ((m = LTR_RUN_RE.exec(s))) {
    ...
    out += esc(s.slice(last, start)) + '<span class="ltr" dir="ltr">' + esc(run) + '</span>';
...
function renderTable(columns, rows, opts = {}) {
  ...
  const body = rows.map(r => `<tr>${r.map((cell, i) => {
    ...
    return `<td class="${numCols.has(i) ? 'num' : ''}">${val(cell)}</td>`;
  }).join('')}</tr>`).join('');
  return `<table class="data"><thead><tr>${head}</tr></thead><tbody>${body}</tbody></table>`;
}

function renderKpis(items) {
  const cells = (items || []).map(k => `<td><div class="k-val">${esc(k.value)}</div><div class="k-lbl">${bidi(k.label)}</div></td>`).join('');
  return `<table class="kpis"><tr>${cells}</tr></table>`;
}
