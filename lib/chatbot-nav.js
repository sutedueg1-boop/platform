...
const HAZARD_DAMAGE = /(مكسور|مكسوره|مخلوع|مقطوع|مكشوف|مكشوفه|تالف|بايظ|باظ|ساقط|واقع|مفكوك|مفكوكه|سايب|متهالك|مصدي|مسرب|تسريب|زلق|شراره|حريق|دخان|ريحه غريبه|ناقص|مش شغال|واقف|مش مثبت|مش مثبته|مش متثبت|مايل|بيهتز|مخلخل|مبلول|فاضيه|منتهي الصلاحيه|مقفول بالغلط)/;
...
function answerNav(text, ctx) {
  ...
  const isHowTo = /(ازاي|كيف|ايه خطوات|ايه متطلبات|متطلبات|شروط|اجراءات|خطوات|قايمه التحقق|بتشتغل ازاي)/.test(nq)
    || /(^|\s)لو\s+(لقيت|شفت|حصل|حصلت|كان|في|فيه)/.test(nq)
    || /(اعمل ايه|اعمل إيه|ماذا افعل|ايه اللي اعمله|المفروض اعمل ايه)/.test(nq);
